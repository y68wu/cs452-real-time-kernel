...
static int test_reroute_command_contract(void) {
        ...
        if (reroute_stage_calls != 1 || current_stage_calls != 0 ||
            ...
            strstr(tc2_ui_status,
                   "reroute staged; enter go to stop safely and replan from CURRENT") == 0) {
                managed_job_state = TC2_JOB_RUNNING;
                managed_job = 0;
                return fail("valid reroute did not stage an operator CURRENT intent");
        }
        ...
        if (reroute_stage_calls != accepted + 1 ||
            !output_contains(
                    "reroute rejected: train must be managed, localized, and not pending removal") ||
            strstr(tc2_ui_status,
                   "reroute rejected; train must be managed, localized, and not pending removal") == 0) {
                managed_job_state = TC2_JOB_RUNNING;
        ...
        if (reroute_stage_calls != accepted + 2 ||
            ...
            strstr(tc2_ui_status,
                   "reroute staged; enter go to stop safely and replan from CURRENT") == 0) {
                managed_job = 0;
                return fail("moving managed train reroute intent was not staged");
        }
