...
static void test_mapping_and_registration(void) {
        for (int number = 1; number <= 18; ++number) {
                int expected = number == 7 ? 4 : number - 1;
                assert(Tc2ConflictZoneForSwitch(number) == expected);
        }
        assert(Tc2ConflictZoneForSwitch(153) == 18);
        assert(Tc2ConflictZoneForSwitch(154) == 18);
        assert(Tc2ConflictZoneForSwitch(155) == 18);
        assert(Tc2ConflictZoneForSwitch(156) == 18);
...
static void test_shared_zone_accepts_independent_ordered_actions(void) {
        tc2_conflict_zone_table table;
        tc2_conflict_zone_request request = {0};
        request.train = 14;
        request.moving = 1;
        ...
        /* Each motor is independent even though both share one zone lock. */
        Tc2ConflictZoneInit(&table);
        request.setting = one_setting(
                5, TC2_CONFLICT_TURNOUT_CURVED);
        assert(Tc2ConflictZoneRequestHard(&table, 4, &request, 0) == 0);

        Tc2ConflictZoneInit(&table);
        request.setting = one_setting(
                7, TC2_CONFLICT_TURNOUT_CURVED);
        assert(Tc2ConflictZoneRequestHard(&table, 4, &request, 0) == 0);

        /* Multiple real actions preserve traversal order and may differ. */
        Tc2ConflictZoneInit(&table);
        request.setting.action_count = 2;
        request.setting.actions[0].switch_number = 7;
        request.setting.actions[0].direction =
                TC2_CONFLICT_TURNOUT_CURVED;
        request.setting.actions[1].switch_number = 5;
        request.setting.actions[1].direction =
                TC2_CONFLICT_TURNOUT_STRAIGHT;
        assert(Tc2ConflictZoneRequestHard(&table, 4, &request, 0) == 0);
        assert(Tc2ConflictZoneTryGrant(&table, 4) == 1);
        assert(table.zones[4].owner_setting.action_count == 2);
        assert(table.zones[4].owner_setting.actions[0].switch_number == 7);
        assert(table.zones[4].owner_setting.actions[0].direction ==
               TC2_CONFLICT_TURNOUT_CURVED);
        assert(table.zones[4].owner_setting.actions[1].switch_number == 5);
        assert(table.zones[4].owner_setting.actions[1].direction ==
               TC2_CONFLICT_TURNOUT_STRAIGHT);

        /* A different physical zone and contradictory duplicates are bad. */
        Tc2ConflictZoneInit(&table);
        request.setting = one_setting(8, TC2_CONFLICT_TURNOUT_STRAIGHT);
        assert(Tc2ConflictZoneRequestHard(&table, 4, &request, 0) == -1);
        request.setting.action_count = 2;
        request.setting.actions[0].switch_number = 5;
        request.setting.actions[0].direction =
                TC2_CONFLICT_TURNOUT_STRAIGHT;
        request.setting.actions[1].switch_number = 5;
        request.setting.actions[1].direction =
...
static void test_fifo_and_approach_priority(void) {
        tc2_conflict_zone_table table;
        uint32_t ticket14;
        uint32_t ticket15;

        /* On one directed track, the physically front train goes first. */
        Tc2ConflictZoneInit(&table);
        ...
        /* Across approaches, a moving arrival beats a stopped arrival. */
        Tc2ConflictZoneInit(&table);
        ...
        /* Stopped contenders use the stable smaller-train recovery rule. */
        Tc2ConflictZoneInit(&table);
        ...
        /* Enqueue order does not change stopped recovery ownership. */
        Tc2ConflictZoneInit(&table);
        assert(Tc2ConflictZoneRequestHard(&table, 4, &second, 0) == 0);
        assert(Tc2ConflictZoneRequestHard(&table, 4, &first, 0) == 0);
        assert(Tc2ConflictZoneTryGrant(&table, 4) == 1);
        ...
        /* A later moving request wins when it is physically closer. */
        Tc2ConflictZoneInit(&table);
...
static void test_soft_prefetch_never_grants_authority(void) {
        ...
        assert(table.zones[4].state == TC2_CONFLICT_ZONE_FREE);
        assert(table.zones[4].owner_train == TC2_CONFLICT_ZONE_NONE);
        assert(Tc2ConflictZoneCanPrefetch(&table, 4, 14) == 0);
        assert(Tc2ConflictZoneCanPrefetch(&table, 4, 15) == 1);
...
static void test_lifecycle_release_and_remove(void) {
        ...
        Tc2ConflictZoneTick(&table, 198U);
        assert(table.zones[4].owner_train == 15);
...
static void test_repeated_zone_requires_a_fresh_grant(void) {
        ...
        assert(Tc2ConflictZoneRequestHard(
                       &table, 4, &request, &repeated_ticket) == 1);
        Tc2ConflictZoneTick(&table, 399U);
        assert(table.zones[4].owner_train == 14);
        Tc2ConflictZoneTick(&table, 400U);
        assert(table.zones[4].state == TC2_CONFLICT_ZONE_FREE);
...
static void test_release_delay_is_exactly_300_ticks(void) {
        tc2_conflict_zone_table table;
        tc2_conflict_zone_request owner = request_for(
                14, 1, 1, 1, 0, 5,
                TC2_CONFLICT_TURNOUT_STRAIGHT);
        ...
        Tc2ConflictZoneTick(&table, 1299U);
        assert(table.zones[4].owner_train == 14);
        assert(table.zones[4].state == TC2_CONFLICT_ZONE_RELEASE_DELAY);
        Tc2ConflictZoneTick(&table, 1300U);
        assert(table.zones[4].state == TC2_CONFLICT_ZONE_FREE);
        assert(Tc2ConflictZoneTryGrant(&table, 4) == 1);
        assert(table.zones[4].owner_train == 15);
...
static void test_central_setting_validation(void) {
        ...
        assert(table.zones[18].owner_setting.actions[0].switch_number == 155);
        assert(table.zones[18].owner_setting.actions[1].switch_number == 153);
...
static void test_central_four_train_fifo_across_both_pairs(void) {
        tc2_conflict_zone_table table;
        ...
        for (int index = 0; index < TC2_CONFLICT_ZONE_MAX_TRAINS; ++index) {
                ...
                assert(Tc2ConflictZoneRequestHard(
                               &table, 18, &request, 0) == 0);
        }

        assert(table.zones[18].hard_count ==
               TC2_CONFLICT_ZONE_MAX_TRAINS);
        assert(Tc2ConflictZoneTryGrant(&table, 18) == 1);
...
static void test_route_plan_branch_merge_and_double_zone(void) {
        ...
        assert(Tc2ConflictZoneBuildRoutePlan(track, &route, 0, 2,
                                             &plan) == 0);
        /*
         * This physical crossing is traversed through both independent
         * actuators.  Preserve the actual BR7-then-MR5 action order; the
         * shared zone must not synthesize or numerically sort the pair.
         */
        assert(plan.step_count == 1);
        assert(plan.steps[0].zone == 4);
        ...
        assert(plan.steps[0].setting.actions[0].direction ==
               TC2_CONFLICT_TURNOUT_CURVED);
        assert(plan.steps[0].setting.actions[1].switch_number == 5);
        ...
        assert(plan.steps[0].last_route_offset == 4);
        assert(plan.steps[0].setting.action_count == 3);
        ...
        assert(plan.steps[0].setting.actions[1].direction ==
               TC2_CONFLICT_TURNOUT_STRAIGHT);
        assert(plan.steps[0].setting.actions[2].switch_number == 154);
        assert(plan.steps[0].setting.actions[2].direction ==
               TC2_CONFLICT_TURNOUT_CURVED);
        assert(plan.steps[0].entry_distance_mm == 246);
        assert(plan.steps[0].exit_distance_mm == 485);
        ...
        assert(Tc2ConflictZoneBuildRoutePlan(track, &route, 0, 5,
                                             &plan) == 0);
        assert(plan.step_count == 1);
        assert(plan.steps[0].setting.action_count == 4);
        assert(plan.steps[0].setting.actions[0].switch_number == 155);
        assert(plan.steps[0].setting.actions[0].direction ==
               TC2_CONFLICT_TURNOUT_CURVED);
        assert(plan.steps[0].setting.actions[1].switch_number == 156);
        ...
        assert(plan.steps[0].setting.actions[2].direction ==
               TC2_CONFLICT_TURNOUT_STRAIGHT);
        assert(plan.steps[0].setting.actions[3].switch_number == 153);
        assert(plan.steps[0].setting.actions[3].direction ==
               TC2_CONFLICT_TURNOUT_CURVED);

        /* Both exit choices of the other motor pair share the same zone. */
        route.node_count = 2;
...
static const tc2_conflict_zone_route_step *crossing_step(
        const tc2_conflict_zone_route_plan *plan) {
        if (!plan) return 0;
        for (int index = 0; index < plan->step_count; ++index) {
                if (plan->steps[index].zone == 4) {
...
static void assert_crossing_curved_traversal(
        const tc2_conflict_zone_setting *setting,
        int first_switch, int second_switch) {
        assert(setting);
        assert(setting->action_count == 2);
        assert(setting->actions[0].switch_number == first_switch);
        assert(setting->actions[0].direction ==
               TC2_CONFLICT_TURNOUT_CURVED);
        assert(setting->actions[1].switch_number == second_switch);
        assert(setting->actions[1].direction ==
               TC2_CONFLICT_TURNOUT_CURVED);
}
...
static void test_a80_d80_crossing_fifo_survives_physical_remove(void) {
        ...
        const tc2_conflict_zone_route_step *a_crossing =
                crossing_step(&a_plan);
        const tc2_conflict_zone_route_step *d_crossing =
                crossing_step(&d_plan);
        assert(a_crossing && d_crossing);
        assert_crossing_curved_traversal(&a_crossing->setting, 7, 5);
        assert_crossing_curved_traversal(&d_crossing->setting, 5, 7);

        tc2_conflict_zone_request train14 = {0};
        train14.train = 14;
        train14.moving = 1;
        train14.track_key = a_crossing->approach_track_key;
        train14.travel_direction = a_crossing->approach_direction;
        train14.front_rank_mm = a_crossing->entry_distance_mm;
        train14.setting = a_crossing->setting;

        tc2_conflict_zone_request train15 = {0};
        train15.train = 15;
        train15.moving = 1;
        train15.track_key = d_crossing->approach_track_key;
        train15.travel_direction = d_crossing->approach_direction;
        train15.front_rank_mm = d_crossing->entry_distance_mm;
        train15.setting = d_crossing->setting;

        Tc2ConflictZoneInit(&table);
        assert(Tc2ConflictZoneRequestHard(
                       &table, 4, &train14, &ticket14) == 0);
        assert(Tc2ConflictZoneRequestHard(
                       &table, 4, &train15, &ticket15) == 0);
        assert(ticket14 != 0 && ticket15 != 0 && ticket14 != ticket15);
        assert(Tc2ConflictZoneTryGrant(&table, 4) == 1);
        assert(Tc2ConflictZoneQueryGrant(&table, 4, &grant) == 1);
        assert(grant.owner_train == 14);
        assert(grant.owner_ticket == ticket14);
        assert_crossing_curved_traversal(&grant.setting, 7, 5);
        ...
        assert(grant.owner_ticket != ticket14);
        assert(grant.state == TC2_CONFLICT_ZONE_GRANTED);
        assert_crossing_curved_traversal(&grant.setting, 5, 7);
        assert(Tc2ConflictZoneMarkSetting(&table, 4, 15) == 0);
        assert(Tc2ConflictZoneMarkSettled(&table, 4, 15) == 0);
        assert(Tc2ConflictZoneQueryGrant(&table, 4, &grant) == 1);
        assert(grant.owner_train == 15);
...
int main(void) {
        ...
        test_shared_zone_accepts_independent_ordered_actions();
        test_fifo_and_approach_priority();
        ...
        test_release_delay_is_exactly_300_ticks();
        test_central_setting_validation();
        test_central_four_train_fifo_across_both_pairs();
