...
/*
 * This image does not save SIMD/FP task state.  Keep the generated track
 * clear scalar even under -O3: a function named memset and a non-volatile
 * byte loop can be recognized as a vectorizable memset idiom by the target
 * GCC despite the freestanding build.
 */
static void zero_track_nodes(track_node *track) {
  volatile unsigned char *p = (volatile unsigned char *)track;
  unsigned int n = TRACK_MAX * sizeof(track_node);
  while(n --> 0) { *p++ = 0; }
}

void init_tracka(track_node *track) {
  zero_track_nodes(track);
  track[0].name = "A1";
