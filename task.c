...
static int abort_ipc_waiter(task_descriptor_t *waiter,
                            task_descriptor_t *receiver,
                            int ipc_error) {
        if (!waiter || waiter == receiver ||
            (waiter->state != TASK_SEND_BLOCKED &&
             waiter->state != TASK_REPLY_BLOCKED)) {
                return 0;
        }

        waiter->sp->x[0] = (uint64_t)(int64_t)ipc_error;
        task_clear_ipc_wait(waiter);
...
int task_exit_descriptor(task_descriptor_t *task, int ipc_error) {
        int aborted = 0;
        task_descriptor_t *queued;

        /*
         * Only the currently running task can execute Exit. Restricting this
         * helper to ACTIVE descriptors guarantees it is absent from every
         * ready/event/IPC wait queue before reclamation.
         */
        if (!task || task->state != TASK_ACTIVE) {
                return -1;
        }
        if (ipc_error >= 0) {
                ipc_error = TASK_IPC_PEER_EXITED;
        }

        /*
         * First detach the explicit SEND_BLOCKED FIFO.  Save send_next before
         * waking a task because task_clear_ipc_wait deliberately severs it.
         */
        queued = task->send_head;
        while (queued) {
                task_descriptor_t *next = queued->send_next;
                aborted += abort_ipc_waiter(queued, task, ipc_error);
                queued = next;
        }
        task->send_head = 0;
        task->send_tail = 0;
        ...
        /*
         * Delivered Send calls are no longer in the FIFO while they wait for
         * Reply, so scan descriptors using the explicit transaction owner.
         * The SEND_BLOCKED check also recovers a waiter if a damaged/stale
         * FIFO link would otherwise leave it behind.
         */
        for (int i = 0; i < MAX_TASKS; ++i) {
                task_descriptor_t *waiter = &tasks[i];
                if (waiter->ipc_receiver != task) continue;
                aborted += abort_ipc_waiter(waiter, task, ipc_error);
        }
