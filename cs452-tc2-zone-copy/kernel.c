...
static task_descriptor_t *io_event_wait_head[EVENT_MAX + 1];
...
static int io_event_valid(int event_type) {
        return event_type == EVENT_UART0_RX || event_type == EVENT_UART0_TX;
}

static int io_event_ready(int event_type) {
        if (event_type == EVENT_UART0_RX) {
                return uart_rx_ready(CONSOLE);
        }

        if (event_type == EVENT_UART0_TX) {
                return uart_tx_ready(CONSOLE);
        }

        return 0;
}

static void io_event_wait_insert(task_descriptor_t *task, int event_type) {
        task->event_type = event_type;
        task->wakeup_time_usec = 0;
        task->event_next = io_event_wait_head[event_type];
        io_event_wait_head[event_type] = task;
}

static int kernel_has_io_event_waiters(void) {
        for (int i = 0; i <= EVENT_MAX; i++) {
                if (io_event_wait_head[i]) {
                        return 1;
                }
        }
...
static void io_release_ready_tasks(void) {
        for (int event_type = 0; event_type <= EVENT_MAX; event_type++) {
                task_descriptor_t **cur;
                if (!io_event_valid(event_type) || !io_event_ready(event_type)) {
                        continue;
                }

                cur = &io_event_wait_head[event_type];
                while (*cur) {
                        task_descriptor_t *task = *cur;
                        *cur = task->event_next;
...
static void kernel_idle_until_timer_event(void) {
        while (kernel_has_timer_waiters() || kernel_has_io_event_waiters()) {
                timer_release_ready_tasks();
                io_release_ready_tasks();
...
void kernel_run(void (*first_task)(), int first_priority) {
        ...
        /*
         * boot.S establishes the C zero-initialization contract for all BSS.
         * Reset the kernel-owned event roots again here so a future alternate
         * entry path cannot reconnect timer/UART wait lists from an earlier
         * run before task descriptors are rebuilt.
         */
        active_task = 0;
        timer_wait_head = 0;
        timer_next_wakeup_usec = 0;
        for (int event_type = 0;
             event_type <= EVENT_MAX; ++event_type) {
                io_event_wait_head[event_type] = 0;
        }
...
context_frame_t *kernel_handle_exception(context_frame_t *frame) {
        ...
        case SYSCALL_EXIT:
                if (active_task) {
                        task_exit_descriptor(
                                active_task,
                                TASK_IPC_PEER_EXITED);
                }
                break;
        ...
        case SYSCALL_SEND: {
                ...
                active_task->ipc_receiver = receiver;
        ...
        case SYSCALL_AWAIT_EVENT: {
                ...
                if (io_event_valid(event_type)) {
                        if (io_event_ready(event_type)) {
                                frame->x[0] = 0;
                        } else {
                                active_task->state = TASK_EVENT_BLOCKED;
                                io_event_wait_insert(active_task, event_type);
                        }
                        break;
                }
        ...
        case SYSCALL_REPLY: {
                ...
                if (sender->state != TASK_REPLY_BLOCKED ||
                    sender->ipc_receiver != active_task) {
                        frame->x[0] = (uint64_t)-2;
                        break;
                }
