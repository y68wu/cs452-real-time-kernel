...
static char* const SYSTEM_TIMER_BASE = (char*)(MMIO_BASE + 0x3000);
...
#define SYSTEM_TIMER_CLO (*(volatile uint32_t*)(SYSTEM_TIMER_BASE + 0x04))
#define SYSTEM_TIMER_CHI (*(volatile uint32_t*)(SYSTEM_TIMER_BASE + 0x08))
...
static void setup_gpio(uint32_t pin, uint32_t setting, uint32_t resistor) {
        ...
        uint32_t status = GPFSEL_REG(reg);
        status &= ~(7u << shift);
        status |=  (setting << shift);
        GPFSEL_REG(reg) = status;

        reg   =  pin / 16;
        shift = (pin % 16) * 2;
        status = GPIO_PUP_PDN_CNTRL_REG(reg);
        status &= ~(3u << shift);
        status |=  (resistor << shift);
        GPIO_PUP_PDN_CNTRL_REG(reg) = status;
}
...
uint64_t timer_get_usec(void) {
        ...
        do {
                hi1 = SYSTEM_TIMER_CHI;
                lo = SYSTEM_TIMER_CLO;
                hi2 = SYSTEM_TIMER_CHI;
        } while (hi1 != hi2);
